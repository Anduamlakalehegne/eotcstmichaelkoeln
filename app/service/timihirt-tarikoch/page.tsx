...
export default function TimihirtTarikochPage() {
  ...
  if (!posts.length) {
    return <div className="text-center text-gray-500 py-8">ምንም የብሎግ ልጥፎች አልተገኙም።</div>
  }
          ...
          <Card key={item.id} className="overflow-hidden flex flex-col">
            ...
            <div className="p-5 flex-1 flex flex-col">
              ...
              <div className="mt-auto flex items-center justify-between">
                <span className="text-xs text-gray-500">ደራሲ፡ {item.author}</span>
                {/* Optionally, add a link to detail page if implemented */}
              </div>
            </div>
          </Card>
        ))}
