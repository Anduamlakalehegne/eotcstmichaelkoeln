...
-- Step 1: UUID helper (works on hosts without extensions)
-- =====================================================
-- Many shared Postgres hosts (e.g., cPanel) do not allow CREATE EXTENSION.
-- We therefore provide a fallback UUID generator using md5 + clock_timestamp.
-- If you have pgcrypto, you can replace generate_uuid() with gen_random_uuid().
